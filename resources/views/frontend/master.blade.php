...
<head>
    ...
    <link rel="stylesheet"
        href="{{ static_asset('frontend/css/materialdesignicons.min.css') }}?version={{ $version }}">
    <link rel="stylesheet" href="{{ static_asset('frontend/css/vue-toastr-2.min.css') }}?version={{ $version }}">


    <link rel="stylesheet"
        href="{{ static_asset('frontend/css/vue-slick-carousel.css') }}?version={{ $version }}">
    <link rel="stylesheet"
        href="{{ static_asset('frontend/css/vue-slick-carousel-theme.css') }}?version={{ $version }}">
    <link rel="stylesheet" href="{{ static_asset('frontend/css/vue-select.css') }}?version={{ $version }}">

    @if (isDemoServer())
        <link rel="stylesheet"
            href="{{ static_asset('frontend/css/color-switcher.css') }}?version={{ $version }}">
    @endif
...
<body class="sg-active">
    ...
    <script>
        ...
        @if (settingHelper('is_tawk_messenger_activated') == 1)
            ...
            (function() {
                ...
                s1.async = true;
                s1.src =
                    'https://embed.tawk.to/{{ settingHelper('tawk_property_id') }}/{{ settingHelper('tawk_widget_id') }}';
                s1.charset = 'UTF-8';
    ...
    @if (isDemoServer())
        <div
            class="sg-yoori-switcher {{ settingHelper('header_theme') == 'header_theme2' ? 'theme-color-switcher' : '' }}">
