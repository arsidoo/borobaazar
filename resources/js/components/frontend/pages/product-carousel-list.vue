<template>
	<div class="flash_sale" :class="{ 'slider-arrows': addons.includes('ishopet') }">
		<VueSlickCarousel v-bind="slick_settings" :rtl="settings.text_direction == 'rtl'" ref="carousel"
			@reInit="swipeSlide">
							...
							<ul class="global-list">
								<li class="product_cart-btn"
									...
									<a href="javaScript:void(0)" @click="cartBtn(product, index)"
										...
										<span class="mdi mdi-name mdi-shopping-outline"></span> <span
											class="btn_text">{{ lang.add_o_cart }}</span>
									</a>
								</li>
								<li v-if="authUser && $store.getters.isThisWishlisted(product.id)"><a
										href="javaScript:void(0)" @click="removeWishlist(product.id)"><span
											class="mdi mdi-name mdi-heart"></span></a></li>
								<li v-else-if="authUser"><a href="javaScript:void(0)"
								...
								<li v-if="compareCheck(product)"><a class="active_compare"
										:class="{ disable_btn: is_disabled == true }" @click="removeCompare(product.id)"
