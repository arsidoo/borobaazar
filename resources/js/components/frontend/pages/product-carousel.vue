<template>
  <div :class="{ 'ishopet-slider-effect slider-arrows': addons.includes('ishopet') }">
    <VueSlickCarousel v-bind="slick_settings" :rtl="settings.text_direction == 'rtl'" ref="carousel_best"
      @reInit="swipeSlide">
      <div v-for="(product, index) in products" :key="index" class="padding_17"
        :class="{ 'ishopet-pr-0': addons.includes('ishopet') }">
        <div class="sg-product slider_div"
          :class="{ 'style-1': type == 'flash', 'ishopet-sg-product': addons.includes('ishopet') }">
          <a :href="'product/' + product.slug" @click.prevent="routerNavigator('product.details', product.slug)">
            <div class="product-thumb">
              <span class="base" v-if="product.special_discount_check > 0">{{
                product.special_discount_type == 'flat' ? priceFormat(product.special_discount_check) + ' ' + lang.off :
              ...
              <ul class="global-list">
                ...
                <div v-if="authUser">
                  <li v-if="$store.getters.isThisWishlisted(product.id)"><a href="javaScript:void(0)"
                      @click="removeWishlist(product.id)"><span class="mdi mdi-name mdi-heart"></span></a></li>
                  <li v-else><a href="javaScript:void(0)" :class="{ 'disable_btn': is_disabled == true }"
                      @click="addToWishlist(product.id)"><span class="mdi mdi-name mdi-heart-outline"></span></a></li>
                </div>
                <li v-if="compareCheck(product)"><a class="active_compare" :class="{ 'disable_btn': is_disabled == true }"
                    @click="removeCompare(product.id)" href="javaScript:void(0)"><span
                      class="mdi mdi-name mdi-scale-balance"></span></a></li>
                <li v-else><a @click="addToCompare(product)" :class="{ 'disable_btn': is_disabled == true }"
                    href="javaScript:void(0)"><span class="mdi mdi-name mdi-scale-balance"></span></a></li>
            ...
            <div class="row"
              ...
              <div class="col-md-12">
                ...
                <div v-else class="btn-group">
                  ...
                  <button class="btn btn-danger border border-1">{{ getCartQuantity(product.slug) }} in bag</button>
