		...
		<section class="grid-view-tab">
			<div class="container">
				<div class="row sg-category-content sg-filter grid-view-tab" :class="activeClass">
					<div class="col-md-4 col-lg-3 filter_area" v-if="showOptions && lengthCounter(productList) > 0">
							...
							<div class="accordion" id="accordionExample">
								<div class="accordion-item" v-if="!category_routes.includes(form.route)">
									<div class="accordion-header" id="ac1">
										<button class="accordion-button" @click="category = !category"
											:class="{ collapsed: !category }" type="button" data-bs-toggle="collapse"
										...
										<div class="accordion-body">
											<ul class="global-list">
												<li v-for="(category, index) in categories" :key="'category' + index"
													...
													<router-link
														:to="{ name: 'product.by.category', params: { slug: category.slug } }">
														{{ category.title }}
														<span class="cat-items">{{ category.total_products }}</span>
													</router-link>
												</li>
											</ul>
											<div v-if="category_load_more || categories.next_page_url"><a
													@click="loadCategories" href="javaScript:void(0)">{{ lang.show_more
													}}</a></div>
										</div>
									</div>
								</div>
								<div class="accordion-item"
									v-if="page && page.child_categories && page.child_categories.length > 0">
									<div class="accordion-header" id="ac2">
										<button class="accordion-button" @click="child_category = !child_category"
											...
											data-bs-target="#child_category" aria-expanded="true" aria-controls="collapse1">
											{{ lang.child_categories }}
										</button>
									</div>
									<div id="child_category" class="accordion-collapse collapse"
										:class="{ show: child_category }" aria-labelledby="ac2">
										<div class="accordion-body">
											<form action="#" class="form-checkbox">
												<ul class="global-list">
													<li>
														<a href="javascript:void(0)" @click="$router.go(-1)"><i
																class="mdi mdi-arrow-left"></i> {{ lang.all_categories
																}}</a>
													</li>
												</ul>

												<div class="form-group"
													v-for="(child_category, index) in page.child_categories"
													:key="'child_category' + index">
													<input type="checkbox" @change="filterProducts"
														:id="'child_category' + child_category.id"
														:value="child_category.id" v-model="form.child_category" />
													<label :for="'child_category' + child_category.id">{{
														child_category.title }}</label>
												</div>
											</form>
										</div>
									</div>
								...
								<div class="accordion-item" v-if="form.route != 'product.by.brand'">
									<div class="accordion-header" id="ac4">
										<button class="accordion-button" @click="brand = !brand"
											:class="{ collapsed: !brand }" type="button" data-bs-toggle="collapse"
											data-bs-target="#brand_collapse" aria-expanded="false"
											aria-controls="brand_collapse">{{ lang.brand }} </button>
									</div>
								...
								<div class="accordion-item">
									<div class="accordion-header" id="ac3">
										<button class="accordion-button" @click="price = !price"
											:class="{ collapsed: !price }" type="button" data-bs-toggle="collapse"
											data-bs-target="#price_collapse" aria-expanded="false"
											aria-controls="collapse2">{{ lang.price }} </button>
									</div>
											...
											<div class="price-top">
												<div class="form-group">
													<input type="text" min="0" :max="min_amount" @focusout="filterProducts"
														v-model="form.price.min" class="form-control"
														:placeholder="lang.min" />
												</div>
												<div class="form-group">
													<input type="text" :min="min_amount" @focusout="filterProducts"
														:max="max_amount" v-model="form.price.max" class="form-control"
														:placeholder="lang.max" />
												</div>
											</div>
											<div class="range-slider">
												<input type="range" min="0" :max="max_amount" step="1"
													@change="filterProducts" v-model="form.price.min" id="min_range" />
												<input type="range" :min="min_amount" :max="max_amount" step="1"
													@change="filterProducts" v-model="form.price.max" id="max_range" />
											</div>
										</div>
									</div>
								</div>
								<div class="accordion-item" v-for="(attribute, index) in attributes"
									:key="'attribute' + index">
									<div class="accordion-header" :id="'attribute_' + attribute.id">
										<button class="accordion-button" type="button" data-bs-toggle="collapse"
											...
											:aria-controls="'attribute_' + attribute.id">{{ attribute.title }} </button>
									</div>
										...
										<div class="accordion-body">
											<form action="#" class="form-checkbox">
												<div class="form-group" v-for="(value, index) in attribute.attribute_value"
													...
													<label :for="attribute.id + 'attribute' + value.id">{{ value.value
													}}</label>
												</div>
											</form>
										</div>
								...
								<div class="accordion-item" v-if="settings.color == 1">
									<div class="accordion-header" id="ac5">
										<button class="accordion-button" @click="color = !color"
											:class="{ collapsed: !color }" type="button" data-bs-toggle="collapse"
									...
									<div id="collapse5" class="accordion-collapse collapse" :class="{ show: color }"
										...
										<div class="accordion-body">
											<form action="#" class="form-checkbox">
												<div class="form-group" v-for="(color, index) in colors.data"
													:key="'color' + index"
													v-observe-visibility="index === colors.data.length - 1 ? handleScrolledForColors : false">
													<input type="checkbox" :id="'filter_color' + color.id"
														@change="filterProducts" :value="color.id" v-model="form.color" />
								...
								<div class="accordion-item">
									<div class="accordion-header" id="ac6">
										<button class="accordion-button" @click="ratting = !ratting"
											:class="{ collapsed: !ratting }" type="button" data-bs-toggle="collapse"
					...
					<div class="col-md-4 col-lg-3 filter_area" v-else-if="showOptions && shimmer">
						<div class="sg-sitebar">
							<shimmer class="mb-3" v-for="(list, i) in 2" :key="i" :height="list == 1 ? 355 : 290"></shimmer>
						</div>
					</div>
					<div :class="showOptions == false ? 'col-md-12 col-lg-12' : 'col-md-8 col-lg-9'">
							...
							<div class="cta mb-4" v-if="page && page.image_835x200">
								<div v-if="page.image_835x200 == getUrl('images/default/default-image-835x200.png')">
									<a v-if="settings.category_default_banner != null"><img loading="lazy"
											:src="settings.category_default_banner" :alt="form.slug"
											class="img-fluid" /></a>
									<a v-else><img loading="lazy" :src="page.image_835x200" :alt="form.slug"
											class="img-fluid" /></a>
								</div>
								<div v-else>
									<a v-if="page.image_835x200"><img loading="lazy" :src="page.image_835x200"
							...
							<div class="d-flex justify-content-between mb-4">
								<div class="left-content">
									<div class="title">
										...
										<button class="btn btn-success mx-2" @click="toggleOptions()"><i
												class="mdi mdi-cog-play-outline"></i></button>
									...
									<div class="d-flex gap-3">
										...
										<ul class="filter-tabs global-list">
											<li class="grid-view-tab" @click="activeClass = 'grid-view-tab'"
												...
												<span class="mdi mdi-name mdi-grid"></span>
											</li>
						...
						<div>
							...
							<div class="show-more" v-if="products && products.next_page_url && !loading">
								<a href="javaScript:void(0)" @click="loadProduct" class="btn btn-primary">{{ lang.show_more
								}}</a>
							</div>
...
export default {
	...
	methods: {
		toggleOptions() {
			this.showOptions = !this.showOptions; // Toggle the showOptions property
		},
		loadCategories() {
			this.category_page++;
		...
		handleScrolledForBrands(isVisible) {
			...
			if (this.brand_page >= this.brand_last_page) { return true }
			this.brand_page++
			this.loadBrands();
		},
